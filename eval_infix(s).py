...
def infixToPostfix(infixexpr):  # fix this code to do floats
    ...
    for token in tokenList:
        if token not in "()+-*/": # before -> if token in "ABCDEFGHIJKLMNOPQRSTUVWXYZ" or token in "0123456789":
...
def postfixEval(postfixExpr):  # also fix this to do floats
    ...
    for token in tokenList:
        if token not in "()+-*/": # before -> if token in "0123456789":
            operandStack.push(float(token)) # before -> operandStack.push(int(token))
        else:
